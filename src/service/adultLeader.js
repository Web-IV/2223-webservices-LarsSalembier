...
const create = ({ personId, yearId }) => {
  ...
  debugLog(`Creating adult leader with personId ${personId} and yearId ${yearId}`);
...
const updateById = (id, { personId, yearId }) => {
  ...
  debugLog(`Updating adult leader with id ${id} to personId ${personId} and yearId ${yearId}`);
  const existingPerson = PEOPLE.find(person => person.id === personId);
  if (!existingPerson) {
    throw new Error(`Person with id ${personId} not found`);
  }
  ...
  const adultLeader = ADULT_LEADERS.find(adultLeader => adultLeader.id === id);
  if (!adultLeader) {
